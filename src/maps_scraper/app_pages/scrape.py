"""
Google Maps'ten canlı veri çekme sayfası.

`webapp.py` bu modüldeki `render()` fonksiyonunu `st.navigation` üzerinden
çağırır. Giriş kontrolü ve `st.set_page_config` merkezi olarak webapp.py'de
yapılır -- sayfa listesi de dahil hiçbir şey giriş yapılmadan oluşturulmaz
(bkz. webapp.py'deki mimari notu).
"""
